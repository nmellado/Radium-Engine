...
RotateGizmo::RotateGizmo( Engine::Component* c, const Core::Transform& worldTo,
    ...
    m_selectedAxis( -1 ) {
    constexpr Scalar torusOutRadius = Scalar( .1 );
    constexpr Scalar torusAspectRatio = Scalar( .08 );
    // For x,y,z
    for ( uint i = 0; i < 3; ++i )
    {
...
Core::Transform RotateGizmo::mouseMove( const Engine::Camera& cam, const Core::Vector2& nextXY,
                                        bool stepped ) {
    static const Scalar step = Ra::Core::Math::Pi / Scalar( 10 );
    ...
    if ( hit1 && hit2 && hits1[0] > Scalar( 0.2 ) && hits2[0] > Scalar( 0.2 ) )
    {
    ...
    } else
    {
        ...
        if ( std::abs( dir( 0 ) ) < Scalar( 1e-3 ) )
        {
            dir << 1, 0;
        } else if ( std::abs( dir( 1 ) ) < Scalar( 1e-3 ) )
        {
            dir << 0, 1;
        } else
